...
pub fn configure<R: tauri::Runtime>(
    ...
    webview.with_webview(move |platform_webview| {
        ...
        view.connect_permission_request(move |view, request| {
            ...
            let wants_camera = request.is_for_video_device();
            if wants_camera && is_trusted_loopback_origin(&current_uri, &origin) {
                request.allow();
            } else {
                request.deny();
            }
...
pub fn configure<R: tauri::Runtime>(
    ...
    webview.with_webview(move |platform_webview| {
        ...
        unsafe {
            ...
            .expect("failed to register the WebView2 camera permission policy");
        };
    })
}
